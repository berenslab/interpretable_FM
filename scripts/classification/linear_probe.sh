...
#SBATCH -J linear_probe                 # Job name
...
for dataset in "${DATASETS[@]}"; do
    ...
    python -m dual_ifm.classification.linear_probe --config-name linear_probe experiment_prefix=tsimcnexb dataset="${dataset}" model.name="${backbone}"

    backbone="tsimcne2d_bagnet33_all_256_1225/tsimcne2d_bagnet33_all_256_1225.pt"
    python -m dual_ifm.classification.linear_probe --config-name linear_probe experiment_prefix=tsimcneb dataset="${dataset}" model.name="${backbone}"

    backbone="simclr_bagnet33_all_256_1000/simclr_bagnet33_all_256_1000.pt"
    python -m dual_ifm.classification.linear_probe --config-name linear_probe experiment_prefix=simclrb dataset="${dataset}" model.name="${backbone}"

    backbone="bagnet33"
    python -m dual_ifm.classification.linear_probe --config-name linear_probe experiment_prefix=imagenetb dataset="${dataset}" model.name="${backbone}"
