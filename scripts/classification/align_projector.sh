...
for dataset in "${DATASETS[@]}"; do
    ...
    echo "Aligning projector for ${dataset}..."
    python -m dual_ifm.classification.align_projector --multirun --config-name align_projector_5fold dataset="${dataset}"
    python -m dual_ifm.classification.eval_embeddings --multirun --config-name align_projector_5fold dataset="${dataset}"
